#  第一节 词汇记忆必备理论基础
##  一、单词记忆最核心问题：

```
1: 英文单词和中文意思建立联系，划上等号，找到感觉。
2: 课后作业练习： hesitate 犹豫不决 之间如何建立联系？

备注：
1）词根: hes- 表示粘
2）词根：it- 表示走
3）后缀：-ate 表示形容词或者动词
```

##  二、词汇记忆方法之词根词缀必背理论基础


前缀举例：
```
1) progress 进步-------------pro-向前 + gress 词根表示走
2）exit 出口-----------------ex-向外 + it 表示走
3）import-（贸易）进口--im- 向内，往里面 + port-表示搬运
4）return –返回-------------- re- 相反，往回 + turn- 转动
```

后缀举例：
```
1）-age(额外扩展，语音里没有) 名词后缀
例： visage 面貌，容貌； ----- vis-词根表示看 + age 名词后缀
2）-al 形容词后缀或者名词后缀
例： personal ----person + al
例： animal------anim + al
3）-ation 名词后缀
例：animation 动画片----anim- 词根表生命 + ation 名词后缀
```

词根举例：
```
1）put- 思考
例：reputation ---re(repeat反复，再一次) + put 思考 + ation 名词后缀
2） vis- 看
例：visual 视力的---vis + ual形容词后缀
3）hes- 粘
例：( 额外扩展)adhesive 粘性的，粘合剂------ad( 加强语气) + hes 粘+ ive 表示形容词或名词
```


#  第2节:词汇记忆初探
##  一. Ambul=walk,表示“行走，走路”
词根助记：ambulance 救护车，流动医院
备注：ambulance 这个单词最原始的意思表示的是战地医院。人抬着单价到处跑，看看哪有伤员。所以ambulance这个单词里面就有个ambul- 表示行走，走路的意思。

照顾基础差同学：以防班级万一有同学不认识ambulance,你可以通过谐音记忆“俺不能死”记住这个单词。然后再进行下面其他单词的记忆。

```
1. Ambulant adj.走动的，流动的
1）词根：ambul- 词根表示走
2）后缀：－ant形容词后缀或者名词后缀，比如important, 比如accountant 会计,比如assistant 助理

2. circumambulate v. 环行，绕…而行
1）前缀：circum-表示圆圈；
比如circumstance环境； circumvent回避（circum- + vent-词根表示走）
2）词根：ambul-表示走
3）后缀：-ate动词或者形容词后缀

3. perambulate v.游历，巡游
1) 前缀：per- 到处，从头到位，从始至终；比如perfect完美——per + fect(词根表示做)
2) 词根: ambul-表示走；
3) 后缀：－ate 表示动词或者形容词后缀；
比如private私下的；considerate考虑周全的；accelerate加速

4. somnambulate /v. 梦游,梦行（额外扩展）
1）词根：somn- 表示睡觉；
比如insomnia 失眠（in- 表示否定＋somn词根表示睡觉＋ia名词后缀）
2）词根：ambul- 表示行走
3）后缀：－ate 动词后缀或者形容词后缀
```

##  二. Anim=life,spirit, 表示“生命，精神”等
词根助记：animal动物
备注：动物就是造物主创造的一种有生命的东西。所以用anim- 生命＋ al名词后缀或者形容词后缀

```
1. animate /v.使有生气，活泼
1) 词根：anim- 表示生命
2) 后缀：-ate 表示动词后缀或者形容词后缀

2. animation n/n. 生动；动画
1) 词根：anim-表示生命
2后缀：－ation名词后缀

3. inanimate adj. 无生命的
1) 前缀：in- 表示否定或者向内
2) 词根：anim- 表示生命
3) 后缀：－ate动词后者形容词后缀

4. magnanimous adj.心胸博大的
1) 前缀：magn- 表示大；
比如：magnify放大magn-表示大＋-ify动词后缀；再比如：classify
2) 词根：anim- 表示生命
3) 后缀：－ous形容词后缀，有多的含义

5. unanimous adj. 意见一致的
1) 前缀：un此处表示的是uni- 表示单一，比如unit 单元； USA
2) 词根：anim-表示生命
3) 后缀：－ous表示形容词后缀

6. animosity n. 憎恨，憎恶
1）词根：anim- 表示生命
2）后缀：-osity复合后缀表名词。其中－ous形容词后缀，－ity名词后缀，比如acidity, stupidity.

练习：——如何建立练习。

7. equanimity n.平静；沉着 (额外扩展)
1）词根：equ-表示相等；
2）词根：anim-表示生命
3）后缀：－ity名词后缀
```


#  第3节:词汇记忆之唤醒动力 让生命飞翔
##  一. av,avar, avi=desire,bird,表示“渴望，鸟”
词根助记：av-鸟——渴望

```
1. avid adj.渴望的，贪婪的
1）词根：av-表示鸟
2）后缀：－id 形容词后缀，比如stupid 愚蠢的； acid 酸的

2. avidity n.贪婪
1）词根：av-表示贪婪，欲望
2）后缀：－idity复合名词后缀；所有以－id 结尾的形容词变名词基本都是在后面直接加－ity
比如：acidity酸度；stupidity愚蠢

3. aviary n.鸟笼，鸟舍
1）词根：av-表示鸟
2）后缀：－ary形容词后缀或者名词后缀
比如：elementary初级的， primary 初级的, imaginary想象的, library 图书馆

4. aviation n.飞行；航空学
1）词根：av-表示飞
2）后缀：－ation名词后缀

5. aviator n.飞行员
1)词根：av-表示鸟（飞行）
2）后缀：－ator复合名词后缀，是由－ate + or复合而成。
```

##  二. bio,bi=life,表示“生命，生物”
词根助记：biology生物； bio + ology学科名词后缀——生物学
再备注：bi-作为前缀表示数字二，比如bicycle 自行车表示的是两个轮子的东西，其中bi-表示二，cycl-词根表示圆圈。Bio-中bi-表示二，o-表示细胞；生命增长和发展形式“二次分裂”。

```
1. biography n.传记
1）词根：bio-表示生命
2）词根：graph-表示写（思考题：graph-这个词根在哪个词中见过？）

2. Biographer n.传记作者
1）词根：bio-表示生命
2）词根：graph-表示写
3）后缀：－er名词后缀，多表人

3. biochemistry ɪ/n.生物化学
1）词根：bio-表示生命
2）词汇：chemistry 化学
备注：口语中chemistry还表示两个人有没有感觉，来不来电。比如你去blind date(相亲)，没成功，朋友问你咋没成功呢？你说We don’t have chemistry. 我们不来电。我没没感觉，我们没有那种feel.

4. autobiography n.自传
1)前缀：auto-表示自动；
2) 词根：bio-表示生命
3)词根：graph-表示写

5. biosphere n.生物圈
1）词根：bio-表示生命
2）词根：sphere-表示圆圈

6. biocide n.杀虫剂
1）词根：bio- 表示生命
2）词根：cid-, cis-表示切，杀，砍等同于cut, 比如decide, suicide.

7. microbe n. 微生物
1)前缀：micro-表示微小
2) 词根：be-等同于bi-表示生命。
```

##  三. Cit=quote,call, 表示“引用，唤起”
词根助记：excited 是由ex + cit + ed构成，兴奋的就是唤起了内心的情绪。Cit- 表示唤起，引起。

```
1. cite v.引用；引证

2.citation n.引用，引证
1）词根：cit-唤起，引起
2）后缀：－ation名词后缀

3. incite v.鼓励
1）前缀：in-两个意思，第一表示向内，第二表示否定，此处表示向内
2）词根：cit-表示引起

4. incitation n.激动
1）前缀：in-表示向内或否定，此处表示宪哥你
2）词根：cit- 表示引起，唤起
3）后缀：－ation

5. recite v.背诵；演奏
1)前缀：re-两个意思，第一个表示往回（return）, 第二个表示反复（repeat）
2)词根：cit-表示唤起，引起

6. recital n.背诵；独奏
1）前缀：re-表示往回，反复
2）词根：cit-表示唤起，引起
3）后缀：－al表示名词或者形容词
```

##  练习1.
```
resuscitate 使(某人或某物)恢复知觉, 苏醒;复活；恢复
其中：sus-表示方向下面
```

#  第4节:词汇记忆之时间一年又一年 背单词的心都不曾改变
上节作业：
```
resuscitate 使(某人或某物)恢复知觉, 苏醒;复活；恢复
其中：sus-表示方向下面
```

本节内容：

##  一. card, cord=heart,表示“心脏，一致”
词根助记：record 记录； re-表示再一次，往回；cord- 词根表示心。最原始的记录就是把…东西或事情一遍一遍的放在心里，完成记忆。
词源助记：因为某些字母发音很像（如：l—n）或者长的很像（如：v—w）在长期的传播过程中容易搞错，产生了音变。换成比较日常的表达就是：产生了互换，产生了通假。比如：Holmes翻译是福尔摩斯。福建人会说自己是胡建人，我们东北人很多人把肉读作you.都是这个原因，都是搞错了。

```
1. cardiac adj.心脏的，心脏病的
1）词根：card-表示心脏；
2）后缀：－ia表示一种病，比如insomnia失眠症；－ac 表示形容词后缀，比如maniac/'meɪnɪæk/ adj. 发狂的

2. cordial adj.首要的，主要的
1)词根：cord- 表示心；
2）后缀：－ial形容词后缀；比如：special

3. cordiality / n.诚心，热诚
1)词根：cord-表示心；
2) 后缀：-ality名词后缀

4. cordiform /adj.心形的
1）词根：cord-表示心；
2）词根：form- 表示形状；比如：transform 变形

5. concord n.协调；协议
1)前缀：con-表示全部，共同，所有
2）词根：cord- 表示心

6. discord n.不和；不一致
1）前缀：dis- 表示否定，或者向四处发散
2）词根：－cord 表示心

7. discordance n.不协调；不一致
1) 前缀：dis-否定
2) 词根：cord- 表示心
3) 后缀：－ance名词后缀
```

##  二、ann, enn=year,表示“年，一年”
词根助记：ann——－an + n(ian)一年；元音可粗略认为可以任意互换；所以ann == enn

```
1. annual adj.一年一度的
1)词根：ann- 表示年
2）后缀：-al 名词后缀或者形容词后缀；

2. anniversary n.周年纪念
1）词根：ann－表示年；
2）词根：vers-表示转动；
3）后缀：－ary表示形容词或者名词后缀

3. annuity n. 年金；养老金
1）词根：ann- 表示年；
2）后缀：－ity名词后缀

4. perennial adj.终年的；长期的
1）前缀：per-表示从头到位，从始至终；
2）词根：enn- 表示年
3）后缀：－ial形容词后缀
```

##  三、chron=time,表示“时间”
词根助记：希腊神话故事中第一代身亡Uranus乌拉诺斯被儿子Chronus用镰刀割掉推翻，后来Chronus害怕被推翻，所以每生一个小孩就把小孩都吃掉。人们认为时间可以吞噬一切，就像克鲁诺斯吞噬他所有孩子一样，所以用Chronus的名字创造词根chron-表示时间。

辅助出现人物：1. Uranus乌拉诺斯（第一代神王）； 2. Zeus宙斯（第三代神王）；3.Aphrodite 阿芙罗狄忒（爱神），又名Venus维纳斯；

```
1. chronic adj. 长期的
1）词根；chron- 表示时间；
2）后缀：－ic形容词后缀

2. chronicle n.编年史
1）词根：chron- 表示时间；
2）后缀：－icle名词后缀；比如article文章

3. chronology n.年代学；年表
1）词根：chron- 表示时间；
2）后缀：－ology 名词后缀，表示一种学科

4. synchronize v.同时发生
1）前缀：syn-等同于sym- 表示相同；
2）词根：chron-表示时间；
3）后缀：－ize动词后缀；

5. synchronous adj. 同期的；同步的
1)前缀：syn-表示相同；
2)词根：chron- 表示时间；
3) 后缀：－ous形容词后缀；
```

练习记忆：
```
chronograph 计时器
```

#  第5节:词汇记忆之只有逃离依附 才能走向光明
上节作业：
chronograph 计时器

本节内容：

##  一. fug=flee,表示“逃，离开” 
词根助记：fu(lly) + g(o)，彻底离开，彻底逃离

```
1. refuge n. 避难，避难所
1）前缀：re- 两个意思，第一个表示return 往回，第二个表示repeat 反复
2）词根：fug- 表示走；

2. refugee n. 逃难者
1）前缀：re－往回，反复
2）词根：fug- 走
3）后缀：－ee名词后缀，表示人；比如：employee 雇员；

3. fugitive adj. 逃亡的 n.逃亡者
1）词根：fug- 表示走
2）词根：it- 表示走
3）后缀：-ive形容词后缀，比如repulsiveadj.反感的，或者名词后缀，比如detective n. 侦探；

4. centrifugal adj.离心的
1）前缀：centr-表示中心（center/centre）
2）词根：fug¬-表示走
3）后缀：-al 形容词后缀或者名词后缀

5. fugacious adj.易散去的，转瞬即逝的
1）词根：fug-表示走
2）后缀：－acious形容词后缀

6. insectifuge/ɪn'sektɪfjuːdʒ/n.驱虫药
1）单词：insect 表示昆虫
2）词根：fug－表示逃跑
```

作业1：
```
lucifugous adj. 避光的
```

##  二. her,hes=stick,表示“粘附” 
词根助记：he man male
she woman female

当时的社会环境和社会条件中，人们认为女性没有办法离开男性单独存在生存，所以根据当时人们的逻辑和认知创造了表示“依附，粘”的词根her-, 以及she的变形hes-

```
1. adhere v.粘着，依附
1）前缀：ad- 加强语气
2）词根：her- 表示粘

2. adherence n.粘附；坚持
1）前缀：ad- 加强语气
2）词根：her- 表示粘
3）后缀：－ence名词后缀

3. adherent n.归依者，门徒, adj. 依附的
1）前缀：ad- 加强语气
2）词根：her- 表示粘
3）后缀：－ent形容词后缀或者名词后缀，偶尔也兼职表示动词后缀

4. adhesion n.粘着；坚持
1）前缀：ad- 加强语气
2）词根：her- 表示粘
3）后缀：－ion名词后缀

5. adhesive adj.发粘的 n.胶带
1）前缀：ad- 加强语气
2）词根：her- 表示粘
3）后缀：－ive形容词后缀或者名词后缀（上文讲过）

6. cohere v.粘着；连贯
1）前缀：co- 表示全部，共同，所有，相当于com-或者con-
2）词根：her- 表示粘

7. coherent adj.连贯的；有条理的
1）前缀：co- 表示全部，共同，所有，相当于com-或者con-
2）词根：her- 表示粘
3）后缀：－ent形容词后缀或者名词后缀，偶尔也兼职表示动词后缀

8. cohesion n.结合，凝聚力
1）前缀：co- 表示全部，共同，所有，相当于com-或者con-
2）词根：her- 表示粘
3）后缀：-ion名词后缀

9. inherent adj.天赋的，生来的
1）前缀：in- 表示向内或者否定
2）词根：her-表示粘
3）后缀：－ent形容词后缀或者名词后缀，偶尔也兼职表示动词后缀

10. hesitate v.踌躇，犹豫
1）词根：hes-表示粘
2）词根：－it表示走
3）后缀：－ate动词后缀或者形容词后缀，偶尔表名词

11. hesitation n. 踌躇，犹豫
备注：一般以－ate 结尾的形容词或者动词，变成名词都把－ate变成－ation.
```

作业2:
```
luc,lust=light,shine,表示“光,照亮” 
1. lucentadj. 发亮的
2. lucid adj.透明的，清楚的
3. lucidity n.光明；明白
4. elucidate v.阐明，解释
5. elucidationn. 阐明，解释
6. translucentadj.半透明的
备注：trans-表示穿透through或者改变
7. noctilucentadj.夜间能发光的
备注：noct-表示夜晚
8. luster n.光泽，光彩
9. lusty adj.精力充沛的
10. lackluster adj. 没有光泽的
11. lustrousadj.有光泽的
12. illustratev.说明，阐明
备注：I + 辅音＋相同辅音表示否定或者加强语气。比如illegal——il + leg + al非法的
13. illustration n. 插图说明
14. illustrious n.辉煌的；著名的
```

#  第6节:词汇记忆之运动流动 带来能量

##  一、fer=bring,carry,表示“带来，拿来” 
词根助记： 1. 通过conference，
2.通过 infer来记，阅读题经常有what can you inferfrom this passage 这种题。

```
1. confer v.协商，谈判
1）前缀：con- 等同于com- 表示全部，共同，所有；
2）词根：fer- 表示带来

2. conference n.商量；会议
1）前缀：con- 等同于com- 表示全部，共同，所有；
2）词根：fer- 表示带来
3）后缀：－ence名词后缀

3. conferment n. 给予
1）前缀：con- 等同于com- 表示全部，共同，所有；
2）词根：fer- 表示带来
3）后缀：－ment名词后缀

4. odoriferous adj.有气味的
1）词根：odor- 表示气味
2）词根：fer- 表示带来
3）后缀：－ous形容词后缀，另外还表示多的含义

5. floriferous adj.有花的
1）词根：flor- 表示花
2）词根：fer- 表示带来
3）后缀：－ous形容词后缀，另外还表示多的含义

6. aquiferous adj.含水的
1）词根：aqu- 表示水
2）词根：fer- 表示带来
3）后缀：－ous形容词后缀，另外还表示多的含义

7. defer v.延期；敬从
1）前缀：de- 表示向下，否定和加强
2）词根：fer- 表示带来

8. deferment n.延期
1）前缀：de- 表示向下，否定和加强
2）词根：fer- 表示带来
3）后缀：－ment名词后缀

9. differ v.不同
前缀：di + 辅音＋相同辅音，从两个辅音中间断开，前面a+辅音部分表示加强语气，强化词根含义。

10. difference n.不同；争论
后缀：－ence名词后缀

11. differentiate v.使有差别；分化
1）词汇different
2) 后缀：－ate 动词后缀或者形容词后缀

12. infer v.推理，推断
1）前缀：in-表示向内；
2）词根：fer- 带来

13. inferable adj.可推知的
1）前缀：in-表示向内；
2）词根：fer- 带来
3）后缀：－able 形容词后缀，表示能够怎么样

14. inferential adj.推理的
1）前缀：in-表示向内；
2）词根：fer- 带来
3）后缀：－ential复合后缀，表示形容词

15. transferable adj.能转让的
1）前缀：trans- 改变，转移
2）词根：fer- 带来
3）后缀：－able形容词后缀
```

练习：
```
circumference n.四周，周围
```

##  二、erg=energy,work,表示“能量，活动” 
词根助记：energy中提取三个字母；erg-表示整个energy能量

```
1. energetic adj. 充满活力的
1）前缀：en- 使，让
2）词根：erg-表示能量
3）后缀：－etic复合后缀，表示形容词。其中－et名词后缀，表示小东西。－ic形容词后缀或者名词后缀

2. energize v. 给予能量
1）前缀：en- 使，让
2）词根：erg-表示能量
3）后缀：-ize动词后缀

3. anergy n.无力
1）前缀：an-表示否定
2）词根：erg- 表示能量
```

小作业：
```
synergic adj.协力的，协同的
1）前缀：syn-等同于sym-表示共同，相同
2）词根：erg- 表示能量；
3）后缀：-ic形容词后缀，或者名词后缀
```

##  三、flu=flow,表示“流动” 
词根助记：1）u-v-w 三个字母音变通假。
2) flu通过流感来记flu表示流动

```
1. fluid n.流体，液体adj. 流动的
1）词根：flu- 表示流
2）后缀：－id形容词后缀，比如acid, stupid等；

2. fluidity n.流动性
备注：－id形容词后缀，变名词＋ity

3. flush v.奔流，冲洗
1）词根：flu-表示流动
2）后缀：－ish动词，名词和形容词后缀，比如：Finish, English and childish.

4. flux n.流动；变迁
备注：flux—就理解成flush 的简单版，声音简化。

5. fluency n.流利，流畅
1）词根：flu-流动
2）后缀：－ency名词后缀
备注：形容词fluent流利的，因为ent- 后缀，变成名词就变成ency or ence

6. affluent adj.流入的；富裕的
1）前缀：a +辅音＋相同辅音，从两个辅音中间断开，前半部分a + 辅音，表示加强语气。
2）词根：flu-流动
3）后缀：－ent此处为形容词后缀

7. affluence n.富裕；财富
备注：-ent变名词ence

8. confluent adj.汇流的，合流的
1）前缀：con-表示全部，共同，所有
2）词根：flu- 表示流动
3）后缀：－ent此处为形容词

9. confluence n.汇流
备注：ent变名词变成了ence

10. effluent adj.流出的 n.流出物
1）前缀：e + 辅音＋相同辅音，从两个辅音中间断开，前半部分表示ex-向外
2）词根：flu- 表示流
3）后缀：－ent此处为形容词和名词后缀

11. effluence n.流出
备注：ent变成名词变为了ence

12. influx n.涌入；汇集
1）前缀：in－表示向内和否定
2）词根：flux 表示流

13. refluent adj. 倒流的
1）前缀：re-表示往回或者反复不断
2）词根：flu- 表示流
3）后缀：－ent此处为形容词后缀
```

练习；
```
superfluous adj.过剩的，多余的
superfluity n.过剩，太多
mellifluous adj. 流畅的；甜美的
备注：melli表示甜美，比如watermelon 西瓜中的mel。
```

作业：
```
equ,equi=equal,even,表示“相等，平均” 

1. equal adj.相等的，平等的
2. equality n.平等
3. equate v.使平等
4. equation n.方程式
5. equable adj. (性情)稳定的；温和的
6. adequacy n.足够
7. equator n.赤道
8. equanimityn.镇静，沉着
9. equilibrium n.平衡，均势
10. equity n.公平，公正
11. equitableadj.公正的，正当的
12. equivalentadj.相等的,等价的
13. equivocal adj.支支吾吾的，模棱两可的
14. iniquityn极不公正;邪恶
```

#  第7节:词汇记忆之别跟不识字的算计 你会有说不出话的痛苦回忆

##  一、Liter=letter,表示“文字,字母”
词根助记：英文单词中有一种东西叫音变。简化版的认识就是：元音可以任意互换，辅音只有通假时候可以换。所以liter== letter. 两个t压缩了一个，变成了词根liter-表示文字；

```
1. literate a.识字的
1）词根：liter- 表示文字；
2）后缀：-ate动词后缀，形容词后缀，或者名词后缀

2. illiterate a.不识字的，文盲的 n. 文盲；
1）前缀：I + 辅音＋相同辅音，从两个辅音中间断开，前半部分表示否定
2）词根：liter- 表示文字；
3）后缀：-ate动词后缀，形容词后缀，或者名词后缀

3. literacy n.认字，有文化
1）词根：liter- 表示文字；
2）后缀：-acy名词后缀，另外ace 也是名词后缀

4. illiteracy n.文盲
1）前缀：I + 辅音＋相同辅音，从两个辅音中间断开，前半部分表示否定
2）词根：liter- 表示文字；
3）后缀：-acy名词后缀，另外ace 也是名词后缀

5. literature n.文学
1）词根：liter- 表示文字；
2）后缀：-ature复合名词后缀，其中由－ate后缀和后缀－ure复合而成

6. literary a.文学的
1）词根：liter- 表示文字；
2）后缀：-ary形容词后缀和名词后缀

7. literal a.文字上的，字面上的
1）词根：liter- 表示文字；
2）后缀：-al 形容词后缀和名词后缀

8. obliterate v.抹掉，擦掉
1）前缀：-ob表示否定,去掉；
2）词根：liter- 表示文字；
3）后缀：-ate此处表示动词后缀

9. obliteration n.删除；灭迹
备注：-ate变成名词-ation.
```

##  二、meter,metr,meas,mens=measure,表示“计量，测量等” 
词根助记： 1. Meter 单词意思是米，是一种测量单位，所以meter-词根表示测量；
2. metr-就是meter的简化版；
3. Meas-通过measure 测量来记；

```
1. barometer n.气压计
1）词根：baro-气压；
2）词根：meter-测量；

2. diameter n.直径
1）前缀：dia-表示在…之间；
2）词根：meter-表示测量

3. perimeter n.周长
1）前缀：per-表示从头到位，从始至终；
2）词根：meter-表示测量；

4. thermometer n.温度计
1）词根：thermo-表示热
2）词根：meter-表示测量；

5. geometry n.几何学
1）词根：geo-表示地
2）词根：metr-表示测量

6. symmetry n.对称；匀称
1）前缀：sym-等同于syn-表示same, 相同；
2）词根：metr-表示测量；

7. asymmetry n.不对称
1）前缀：a-表示ab- 否定，或者ad-加强语气；
2）前缀：sym-相同；
3）词根：metr-表示测量；

8. immeasurable adj.不能测量的
1）前缀：im-表示否定；
2）词根：meas-表示测量
3）后缀：－urable复合后缀，由－ure和－able复合而成。

9. immense adj.无限的，广大的
1）前缀：im-表示否定；
2）词根：mens-表示测量
```

作业：
```
Log,logu=speak,表示“说话”

1.logic n.逻辑（学）
2.logical a.符合逻辑的
3.analogy n.类比；类似
4.apologue n.寓言
5.apologize v.道歉
6.catalogue n.目录,一览表
7.dialogue n.对话
8.prologue n.开场白
9.epilogue n.尾声，后记
10.eulogy n.颂辞，称赞
11.eulogize v.称赞
12.monologue n.独白

memor=memory,表示“记忆” 

1.memory n.记忆，记忆力
2.memorize v.记住，熟记
3.immemorial a.远古的；无法追忆
4.memorandum n.备忘录
5.commemorate v.纪念
6.commemorable a.值得纪念
7.remember v.回忆；记住
8.remembrance n.回忆；记忆力
```

#  第8节:词汇记忆之社会关系要搞清楚 不能无法无天整天打打杀杀
上节作业抽讲：
```
1. catalogue n.目录,一览表
1）前缀：cata-在下面；
2）词根：log-说话；
3）后缀：-ue结尾表示名词后缀，比如dialogue对话，比如value 价值；

2. prologue n.开场白
1）前缀：pro-表示向前；
2）词根：log-说话；
3）后缀：-ue结尾表示名词后缀

3. epilogue n.尾声，后记
1）前缀：epi- 表示向后；
2）词根：log-说话；
3）后缀：-ue结尾表示名词后缀

4. eulogy n.颂辞，称赞
1）前缀：eu- 表示好；
2）词根：log- 表示说话；

5. monologue n. 独白
1）前缀：mono表示单一
2）词根：log-说话；
3）后缀：-ue结尾表示名词后缀
```

本节课内容：
##  一、soc=companion,表示“同伴”，引申为“社会” 
词根助记：通过society或者social 来记。社会本质从某一角度来说就是相互练习的整体。马克思把人定义为“人就是社会关系的总和。”在社会上，不可能孤立存在，和其他人完全脱离联系。所以soc-词根记成“联系，社会”。

```
1. sociable adj.爱社交的
1）词根：soc-表示社会，联系；
2）后缀：－able 形容词后缀；

2. socialism n.社会主义
1）词根：soc- 表示社会，联系；
2）后缀：－ial形容词后缀；
3）后缀：－ism 名词后缀，表示“主义”比如capitalism资本主义；

3. sociality n.爱交际
1）词根：soc- 表示社会，联系；
2）后缀：－ial形容词后缀；
3）后缀：－ity名词后缀，而且以－al或者-ial结尾的形容词变成名词的时候多在后面直接加－ity

4. sociology n.社会学
1）词根：soc-表示社会，联系；
2）后缀：－ology 学科名词后缀；

5. associate v.联合；联想
1）前缀：a +辅音＋相同辅音，从两个辅音中间断开，前半部分表示加强语气，或翻译成使，让；
2）词根：soc-表示联系；
3）后缀：－ate 多表动词和形容词后缀；

6. association n.社团；联合
备注：－ate变名词直接变为－ation

7. consociate v.联合的
1）前缀：con- 等同于com- 表示全部，共同，所有；
2）词根：soc-表示社会，联系；
3）后缀：－ate动词后缀或形容词后缀；
备注：我语音中好像口误说它是形容词和动词，如果我说它形容词请更正为动词。不好意思。

8. dissociate v.分离;断绝关系
1）前缀：dis-表示否定，或者向四处发散；
2）词根：soc-表示联系；
3）后缀：－ate动词后缀或形容词后缀；
```

##  二、clear, clar, clair=clear, bright,表示“清楚，明白”
词根助记：这个词根变来变去不就是clear 元音互换嘛。所以你不用管他怎么变。只要cl—r中间夹杂元音字母就是clear表示清楚明白；

```
1. clearance n.清算；清除
1）词根：clear- 表示清楚，明白；
2）后缀：－ance名词后缀

2. clarify v.澄清，弄清
1）词根：clar- 表示clear清楚明白；
2）后缀：－ify动词后缀。比如classify分类；

3. clarity n.清楚
1）词根：clar- 表示清楚明白；
2）后缀：-ity名词后缀；

4. declare v.宣称；断言
1）前缀：de- 表示向下，否定和加强，在文中表示的是加强语气；
2）词根：clar-表示清楚，明白；

5. declaration n.定言；陈述
1）前缀：de-表示向下，否定和加强；
2）词根：clar-表示清楚，明白；
3）后缀：－ation名词后缀；

6. clairaudient 有超人听力的
1）词根：clair-表示清楚明白；
2）词根：aud-表示听；
3）后缀：－ent此处为名词后缀
```

作业：
```
一、cid，cis=cut,kill,表示“切开，杀”
助记：decide决定，决心
decision决心，决议
suicide自杀

1.homicide n.杀人
2.insecticide n.杀虫剂
3.pesticide n.杀虫剂
4.circumcise v.行割礼，割包皮
5.concise adj.简明的
6.excise v.割去，删去
7.excision n.删除，灭绝
8.incisive adj.一针见血的，锐利的
9.incisor n.门牙
10.precise adj.精确的，清楚的
11.precision n.精密，正确

二、astro,aster=star,表示“星星，天空”
词根助记：star 简单单词变形，字母顺序变化变成astr-表示天空，星星。
disaster其中dis-表示否定，aster- 表示星星；古代认为，星位不正，必将有大事发生。你看电视里面演皇帝去世之前，都要有一颗流行的，大概就是这个概念。

1. astronomy n.天文学
2. astrospace n.宇宙空间
3. astronaut n.宇航员
4. astrophysics n.天体物理学
5. astral adj.星的，星状的
6. disastrous adj.灾难的
```

#  第9节:词汇记忆之生活中正直爱笑 一切会变的很美好 连呼吸都很顺畅
 
##  一、spir=breathe,表示“呼吸”
词根助记：神话故事中普罗米修斯创造人类后，人类只是一坨直立行走的人行泥巴，没有智慧，没有呼吸，没有精神，也没有灵魂。后来普罗米修斯请他的好朋友雅典娜（Athena）来帮助他让人类变得更有智慧，让人有精神，雅典娜对所有泥人吹乐一口气，人就有带着呼吸行走，有乐精神。也就是spirit---spir + it，其中spir-表示呼吸；

```
1. aspire v.热望；立志
1）前缀：a-加强语气；
2）词根：spir-表示呼吸；
 
2. aspiration n.渴望，热望
备注：直接在aspire 后加－ation名词后缀
 
3. aspiring adj.有抱负的
备注：直接在aspire后加－ing表形容词；
 
4. conspire v.同谋
1）前缀：con-表示全部，共同，所有
2）词根：spir-表示呼吸；
 
5. conspiracy n.共谋；反叛
1）前缀：con-表示全部，共同，所有
2）词根：spir-表示呼吸；
3）后缀：－acy名词后缀
 
6. expire v.断气；期满
1）前缀：ex-表示向外；
2）词根：spir-表示呼吸，因为前面x发音中有s,被简化为pir-
 
7. expiration n.断气；期满
1）前缀：ex-表示向外；
2）词根：spir-表示呼吸，因为前面x发音中有s,被简化为pir-
3）后缀：－ation名词后缀
 
8. inspire v.吸气；鼓励
1）前缀：in- 表示向内；
2）词根：spir-表示呼吸；
 
9．Inspiration n.吸气；灵感
1）前缀：in- 表示向内；
2）词根：spir-表示呼吸；
3）后缀：－ation名词后缀
 
10. perspire v.出汗
1）前缀：per-表示全部，共同，所有；
2）词根：spir-表示呼吸
 
11. perspiration n.出汗
1）前缀：per-表示全部，共同，所有；
2）词根：spir-表示呼吸
3）后缀：－ation名词后缀
 
12. respire v.呼吸；恢复精神
1）前缀：re-表示往回贺反复不断；
2）词根：spir-表示呼吸；
 
13. respiration n.呼吸
1）前缀：re-表示往回贺反复不断；
2）词根：spir-表示呼吸；
3）后缀：－ation名词后缀
 
14. dispirit v.使沮丧
1）前缀：dis-否定
2）词根：spirit精神
```
 
小作业：
```
transpire v.泄露
spiritual adj.精神的；宗教的
```
 
##  二、rect=stright, right, 表示“正，直”
 
词根助记：
1. 通过熟悉词correct adj.正确的, 来记忆。其中co+辅音＋相同辅音表示全部，共同，所有。
2. 词源角度来看，right中rig-就是rect，因为g-ct通假关系。比如act 通过这个通假关系，产生词根ag-表示做。

```
1. rectify v.改正
1）词根：rect-表示正、直
2）后缀：－ify动词后缀；
 
2. rectitude n.正直；公正
1）词根：rect-表示正、直
2）后缀：－itude抽象名词，比如attitude 态度；
 
3. rectangle n.矩形
1）词根：rect-表示正、直
2）词汇：angle 表示角度
 
4. erect adj.直立的; v.竖立
1）前缀：e-表示ex-向外；
2）词根：rect-表示正、直
 
5. erection n.直立，竖立
1）前缀：e-表示ex-向外；
2）词根：rect-表示正、直
3）后缀：－ion 名词后缀
 
小作业：
correctitude n.行为端正
```

作业：
```
rid, ris= laugh,表示“笑”
 
1.ridiculous adj.可笑的
2.ridicule v.嘲笑
3.deride v.嘲笑
4.derisive adj.嘲笑的；可笑的
5.risible adj.爱笑的
6.risibility n.爱笑，能笑
```

#  第10节:词汇记忆之只写下愿望 移动行走到太空
 
##  一、scrib,script=write,表示“写”
词根助记：
1. describe v.描写，叙述； de+scrib+e
2. description n.描绘，形容
 
```
1.scribe n.抄写员
就是词根：scrib + e 表示写东西的人
 
2. script n.原本，脚本
就是词根：script 表示写的东西；
 
3. scripture n.圣经，经典
1）词根：script-表示写；
2）后缀：－ure名词后缀
 
4. circumscribe v.立界限；限定
1）前缀：circum-表示圆圈；
2）词根：scrib-表示写
 
5. conscribe v.征募，招募
1）前缀：con-表示全部，共同，所有；
2）词根：scrib-表示写；
 
6. conscript n.壮丁，新兵
1）前缀：con- 表示全部，共同，所有
2）词根：script-表示写；
 
7. inscribe v.铭刻
1）前缀：in- 表示向内；
2）词根：scrib- 表示写；
 
8. inscription n.铭刻；碑文
1）前缀：in-表示向内；
2）词根：script- 表示写
3）后缀：－ion 名词后缀
 
9. manuscript v.手抄本；原稿
1）前缀：manu-表示手；比如manual, manufacture
2）词根：script-写；
 
10. postscript n.(信等)附言
1）前缀：post-在后面
2）词根：script-表示写；
 
11. prescribe v.开药方；命令
1）前缀：pre-在…前面；
2）词根：scrib- 写；
 
13. prescription n.处方；命令
1）前缀：pre- 在…前面；
2）词根：script-表示写；
3）后缀：－ion名词后缀
 
14. subscribe v.订阅；认购
1）前缀：sub-在下面；
2）词根：scrib-表示写；
 
15. subscriber n.订户；捐助人
1）前缀：sub-在下面；
2）词根：scrib-表示写；
3）后缀：－er表示人；
 
16. transcribe v.抄写，誊写
1）前缀：trans- 表示转移，改变；
2）词根：scrib-表示写
 
17. transcript n.副本，抄本
1）前缀：trans-表示转移，改变；
2）词根：script- 表示写；
```

##  二、stell=star,表示“星星”
音变：l—m-n-r
词根助记：
stell---star因为音变l—m—n—r ，所以stell==star

```
1. stellar adj.星的
1）词根：stell-星星；
2）后缀：－ar名词后缀或者形容词后缀
 
2. interstellar adj.星际的
1）前缀：inter-在…之间；比如：interview 面试，采访——－都是对着看；
2）词根：stell- 表示星星；
3）后缀：－ar此处为形容词后缀
 
3. constellate v.形成星座；群集
1）前缀：con-表示全部，共同，所有
2）词根：stell- 表示星星；
3）后缀：－ate 动词或形容词后缀
 
4. constellation n.星座
1）前缀：con- 表示全部，共同，所有
2）词根：stell- 表示星星
3）后缀：－ation名词后缀
 
5. stellify v.使成明星
1）词根：stell-表示星星；
2）后缀：－ify动词后缀
 
6. stelliform adj.星形的
1）词根：stell-星星；
2）词根：form- 表示形状
```

##  三、mob=move,表示“动”
词根助记：mobile adj. 活动的，可动的；大家可以用China Mobile 中国移动来记mob- 词根。

```
1. immobile adj.不动的
1）前缀：im-表示否定或者向内；
2）词根：mob-表示动
3）后缀：－ile形容词或者名词后缀；比如：juvenile 青少年的；missile 导弹；
 
2. mobility n.能动性
1）词根：mob- 表示动
2）后缀：ility名词后缀
 
3. mobilize v.动员
1）词根：mob- 表示动；
2）后缀：－ilize动词
 
4. automobile n.汽车
1）前缀：auto-自动
2）词根：mob-表示移动；
3）后缀：－ile形容词或者名词后缀
 
5. mob n.暴民
 
6. mobbish adj.暴乱的
1）词根：Mob- 表示动
2）后缀：－ish此处为动词后缀，有动词名词和形容词
 
7. mobster n.匪徒，歹徒
1）词根：mob-表示动；
2）后缀：－(i)ster表示人；
```

作业：
```
vad,vas = go，表示“走” 

1. evade v.逃避；偷税
2. evasion n.逃避；借口
3. evasive adj.逃避的
4. invade v.侵略
5. invasion n.侵略
6. pervade v.蔓延；普及
7. pervasion n.蔓延，弥漫
8. pervasive adj.普及的，渗透
```

#  第11节:词汇记忆之多听多看长智慧
 
##  一. Soph=wise,表示“智慧，聪明”
词根助记：sophomore n.大学二年级学生
sophie苏菲的世界

```
1. sophist n.博学之人；诡辩家
1）词根：soph- 智慧
2）后缀：－ist表示人；
3）熟词强化：
scientist科学家；artist 艺术家；dentist 牙医
 
4）常见词扩展：
feminist n.女权主义者；
antagonist n.敌手，对手
 
2. sophism n.诡辩
1）后缀：-ism名词后缀，表示…主义
2）熟词强化：socialist社会主义者；capitalist 资本主义着；materialism物质主义；
3）常见词扩展：atheism n. 无神论
 
3. sophisticate v.使精致；使老于世故
1）后缀：isticate复合后缀；
2）熟词强化：fanatic adj.狂热的； critic n.批评家；
3）常见词扩展：apathetic adj.无感情的；冷淡的
 
4. sophisticated v.老于世故的；精致复杂的
1）后缀：-ed形容词；
 
5. philosophy n.哲学
1）词根：philo-表示爱；
2）词根助记：philo—vilo—lovi—love
```

##  二. spect, spic= look, see,表示“看”
词根助记：expect v.期待，盼望
expectation n.期待，希望
respect n.尊敬

```
1. aspect n.样子，容貌；方面
1）词根：a- 两个意思，第一个表示ad-加强语气；第二个表示ab- 否定；
 
2. circumspect adj.慎重的
1）前缀：circum-表示圆圈；
2）熟词强化：circumscribe 限制；circumcise 环切；
3）常见词扩展：circumvent v.回避，设法规避；
 
3. circumspection n.慎重；细心
 
4. inspect v.检查；视察
前缀：in- 表示向内，或者否定；
 
5. inspection n.视察；认真看
 
6. prospect n.展望，前景
1）前缀：pro-表示向前；
2）熟词强化：prologue 前言；
3）常见词扩展：propel v.推动；
 
7. prospective adj.将来的
1）后缀：－ive形容词后缀；
2）熟词强化：naive 天真的；native 本地的；本地人；fugitive逃亡者；
3）常见词扩展：pervasive adj,遍布的，无处不在的；repulsive adj.厌恶
```


#  第12节:词汇记忆之只看庞麦隆跳舞 摩擦摩擦
接上节：
 
##  二. spect, spic= look, see,表示“看”

```
7. prospective adj.将来的
1）后缀：－ive形容词后缀；
2）熟词强化：naive 天真的；native 本地的；本地人；fugitive逃亡者；
3）常见词扩展：pervasive adj.遍布的，无处不在的；repulsive adj.厌恶的；
 
8. prospectus n.计划书；大纲
1）后缀：-us 名词后缀；
2）前缀记忆故事：pro-以及epi
3）常见词扩展：
procedure n. 程序
prognosis n. 预测
consensus n. 一致，同意
 
9. respectable adj.可敬的
 
10. respectful adj.充满敬意的
 
11. retrospect v.回顾，回想
1）前缀：retro-表示往回；
2）前缀记忆：intro---retro对比
3）常见词扩展：
retrograde 退化，恶化；
retrogress v.倒退，逆行
 
12. retrospective adj.回顾的；怀旧的
 
13. suspect v.怀疑
1）前缀：sus-表示在…下面；
2）常见词扩展：suspend n. 暂停
 
14. suspicion n.怀疑
 
15. suspicious adj.多疑的
1）后缀：-ous形容词后缀；
2）熟词强化：
lustrous adj.有光泽的；dangerous危险的；mountainous 多山的；
 
16. spectacle n.奇观；状观；
1）后缀：-acle名词后缀；
2）熟词强化：miracle奇迹； obstacle障碍；
3）常见词扩展：manacle 手铐；
 
17. spectacular a.奇妙的；惊人的
1）后缀：－ar形容词或名词后缀
2）熟词强化：liar撒谎的人，beggar 乞丐；familiar 熟悉的
3）常见词扩展：linear adj.线性的；linear algebra 线性代数；
 
18. spectator n.观众，旁观者
1）后缀：－or 名词后缀；表示人；
2）扩展：表示人的后缀，－er, -ant, -ist, -ese
 
19．speculate v.思索；投机
1）后缀：－ate动词后缀
2) 熟词强化：accelerate加速；
3）常见词扩展：
innate adj. 天生的，固有的；
moderate adj.中等的
delegate v. 代表
 
20. speculation n.思索；投机
 
21. conspicuous adj,显眼的，引人注目的
1）前缀：con- 全部，共同，所有
2）后缀：－ous形容词后缀
 
22. despicable adj.可鄙的
1）前缀：de-表示向下；
2）词根；spic-表示看
 
23. despise v.轻视，看不起
词根：spis- 等同于spic-因为s-c发音像，而产生音变；
 
24. perspicacious adj.有洞察力的
1）前缀：per- 表示穿过，透过
2）后缀：acious复合形容词后缀
```

##  一. ras, rad=scrape,表示“刮擦”
词根助记：
erase v.擦掉
eraser n.橡皮；黑板擦

```
1. abrade v.磨损
1）前缀：ab- 加强语气；
2）词根：rad- 表示刮擦；
 
2. abradant n.砂纸
1）后缀：－ant形容词后缀或者名词后缀
2）熟词强化：important重要的；extravagant n.浪费的
3）常见词扩展：exuberant adj.茂盛的，生机勃勃的
 
3. abrasive adj.有打磨作用的
4. raze v.铲平，摧毁
5. razor n.剃刀
```

#  第13节:词汇记忆之不要限制尝试 多去体验各种情绪
 
##  一、limin,lim=threshold，表示“门槛，限制”
词汇助记：
1. Limit n.限制来记。Lim-表示限制＋ it表示走
2. lim- 词源等同于line 一条线。

```
1．eliminate v.消灭；删除
1）前缀：e- 表示ex-向外；
2）熟词强化：exit出口；export 出口
3）常见词扩展：
exalt v.拔高，提高
expel v. 驱逐
expurgate v.删除，修订，删去不当之处
 
2. preliminary a.预备的;初步的
1）前缀：pre向前；
2）词根：limin-表示限制门槛；
3）后缀：－ary形容词后缀或者名词后缀；
4）熟词强化：
imaginary 想象的；primary 初级的；elementary 初级的；anniversary 周年纪念
 
5）常见词扩展：
stationary adj.静止的，不动的；
mercenary adj.唯利是图的
 
3. subliminal a.潜意识的
1）前缀：sub- 在下面；
2）后缀：－al 形容词或者名词后缀；
3）熟词强化：personal个人的；animal 动物；
4）常见词扩展：
congenital adj.天生的，先天的；
rival n.竞争对手
 
4. supraliminal a.有意识的
1）前缀：supra- 表示上面
2）常见词扩展：
superficial adj.表面的；
supervise v. 监督，管理
 
5. sublime a.崇高的
1）前缀：sub在下面；
2）词根：lim- 等同于line表示限制
 
6. sublimate v.升华；提高
后缀：－ate 动词后缀或形容词后缀
 
7. delimit v.定界限
1）前缀：de- 表示向下，否定和加强；
2）常见词扩展：
depict v. 描绘
defame v.诽谤，中伤；
```

##  二、per=try,表示“尝试”
词根助记：
experience n.经验，经历 –ex + per + i + ence
experimentn.实验 --- ex + per + I + ment

```
1. peril v.冒险; n.危险
1）后缀：il
2）后缀：-ile形容词或名词后缀；
3）熟词强化：juvenile青少年的；missile导弹；
4）常见词扩展：
tortile adj.弯曲的，扭曲的；
senile adj.老年的
 
2. perilous a.危险的
后缀：ilous复合后缀，表形容词
 
3. empiric n.经验主义者
1）前缀：em-进入，使；
2）词根：pir- 表示尝试，等于per-
3）后缀：-ic形容词后缀或名词后缀
4）常见词扩展：antibiotic adj. 抗生素的；n. 抗生素
 
4. empirical a.靠经验办事的
 
5. empiricism n.经验主义
```

##  作业：
```
path=feeling, suffering, illness,表示“感情，痛苦，病”
pathetic adj.可怜的；动情的
antipathy n.反感
apathy n.冷淡，漠不关心
apathetic adj.冷漠的
sympathy n.同情，怜悯
pathology n.病理学
psychopath n.精神病人
electropathy n.电疗法
```

#  第14节:词汇记忆之要相信语言的力量
 
##  一、Lingu=language ,表示“语言”，原意为“舌头”
词根助记：language语言。其中－age 名词后缀，把后缀－age 去掉后，剩下langu-就是表示语言。然后元音稍微变了一下变成了lingu-语言。

```
1. linguist n.语言学家
1)后缀：－ist表示人
2）熟词强化：feminist女权主义者；antagonistn敌手，对手；
3）常见词扩展：arsonist n.纵火犯
 
2. linguistics n.语言学
1）后缀：－ics学科名词后缀
2）熟词强化：economics n.经济学; physics n.物理学；
3）常见词扩展：
pediatrics n.小儿科
genetics n.基因学
 
3. bilingual a.两种语言的
1）前缀：bi-表示二
2）熟词强化：bicycle自行车；
3）常见词扩展：recycle v. 回收利用
 
4. unilingual a.一种语言的
1）前缀：uni-表示单一；
2）熟词强化：unit 单元；uniform制服；
3）常见词扩展：unison n. 和谐，同音；
 
5. multilingual adj.懂多种语言的
1）前缀：multi-表示多
2）熟词强化：multiply乘；multimedia 多媒体；
3）常见词扩展：multiparous adj.一胎多子的
 
6. sublingual adj.舌下的
```

#  二、fid=trust,faith,表示“相信，信念”
词根助记：confidencen.自信，信任
confident adj.自信的

```
1. fidelity n.坚贞；忠心
1）后缀：-el名词后缀；表示人或物；
2）熟词强化：novel小说；model 模特；
3）常见词扩展：
personnel n.全体工作人员；
parcel n. 包裹；
 
2. confide v.吐露；信托
1）前缀：con- 表示全部，共同，所有
2）熟词强化：conflict 打，冲突；confluence汇流；
3）常见词扩展：consolidate v. 巩固，加强
 
3. confidant n.知心朋友
1）后缀：-ant 形容词后缀或者名词后缀
2）熟词强化：important 重要的；accountant 会计；
3）常见词扩展：
exuberant adj. 茂盛的，兴高采烈的
extravagant adj.奢侈的
 
4. confidential a.秘密的，机密的
1）后缀：-ial形容词后缀
2）熟词强化：special adj. 特殊的；
3）常见词扩展：
impartial adj. 公正的，不偏不倚的
beneficial adj. 有益的，有好处的
 
5. diffident adj.自卑的，胆怯的
1）前缀：di +辅音＋相同辅音，从两个辅音中间断开，前半部分表示否定
2）常见词扩展：diffuse 扩散，传播
 
6. perfidy n.不忠实，背叛
1）前缀：per－表示从头到尾，从始至终，第二个意思表示假，坏
2）熟词强化：pervert 变态；
3）常见词扩展：perjury n.伪证;假誓
 
7. perfidious adj.背信弃义的
 
8. infidel n.没信仰的人；异教徒
1）前缀：in－表示否定
2）后缀：－el 名词后缀；
 
9. affidavit n.口供书；宣誓书
后缀：－avit表示名词
```

本节作业：
##  三、doc,doct=to teach, 表示“教”
```
词根助记：
doctor n 博士，医生
document n.文件；证书
 
1. doctoral a.博士的；权威的
1）词根：doc-
2）后缀：-or 名次后缀，－al 形容词后缀和名词后缀
3）常见词巩固：
 
2. doctrine n.教义，教条
1）后缀：－ine形容词后缀和名词后缀；
2）熟词强化：marine海军的；routine例行公事；
3）常见词扩展：famine n.饥荒
 
3. doctrinaire n.空谈家 a.教条的
1）后缀：-aire表名词
2）熟词强化：millionaire百万富翁；questionnaire 调查问卷；
 
4. docile adj.可教的；温顺的
1）后缀：－ile形容词后缀或名词后缀
2）熟词强化：senile 老年的；
3）常见词扩展：tortile adj.扭曲的；
 
5. indocile adj.难驯服的
```

#  第15节:词汇记忆之运打破世俗束缚 保持纯洁心灵
 
 
##  一、Fract=break,表示“打碎”
词根助记：b-p-f-v音变关系；ct-k-g音变关系；fract =break
 
```
1. fraction n.部分；小片，分数
1）词根：fract-表示break 打破，打碎；
2）后缀：－ion名词后缀；
3）熟词复习：concision n. 简洁；
4）常见词扩展：
consolation n.安慰，慰问
convocation n.召集
 
2. fractional adj.碎片的
1）后缀：－al形容词后缀；或名词后缀；
2）熟词强化：empirical adj.以经验为依据的
3）常见词扩展：fatal adj. 致命的；
 
3. fractious adj.易怒的，脾气坏的
1）后缀：－ious形容词后缀；
2）常见词扩展：
prosperous adj.繁荣的
circuitous adj.迂回的
 
4. fracture n.骨折；断裂
1）后缀：－ure名词后缀；
2）常见词扩展：
torture n.酷刑，折磨
expenditure n.支出
signature n.签名
 
5. diffract v.分解
6. infraction n.违反，违背
 
7. refract v.折射
1）前缀：re-表示往回，反复；
2）常见词扩展：
 
reverse 反转的,颠倒的
repugnant adj.令人厌恶的
resonant adj.回响的;洪亮的
 
8.refraction n.折射作用
 
9.refractory adj.倔强的；不听话的
1）后缀：－ory名词后缀或形容词后缀
2）熟词强化：contradictory adj.矛盾的；armory n.军械库
3）常见词扩展：compulsory adj.强制性的
inventory n.财产清单，概括
 
10. fragile adj.易碎的
1）后缀：-ile形容词后缀和名词后缀；
2）熟词强化：tortile adj.扭曲的；domicilen.住所
3）常见词扩展：
servile adj.奴性的，过度屈从，缺乏独立性的；
ductile adj.可塑的;可延展的, 有韧性的;顺从的, 易受影响的
 
11. fragility n.虚弱；脆弱
1）后缀：-ity名词后缀；
2）熟词强化：equanimity n.镇定，沉着
3）常见词扩展：duplicity n.欺骗；口是心非
 
12. fragment n.碎片；片断
1）后缀：-ment名词后缀；
2）常见词扩展：segment n.部分；
garment n.衣服
```

##  二、pur, purg=pure, 表示“纯洁”
词根助记：1. 通过简单单词pure记忆。
2. purge v. 使清净
 
```
1. purity n.纯净，纯正
2. impure a.不纯的
3. impurity n.不纯；有罪
4. purify v.使清洁；使纯正
1）后缀：－ify动词后缀；
2）熟词强化：classify 分类；
3）常见词扩展：
dignify v. 使高贵，使显赫；
fortify v.加固，加强
 
5. purist n.纯粹主义者
6. puritan n.清教徒
7. purgation n.清洗；吃泻药
8. purgative a.清洗的 n. 泻药
1）后缀：-ative形容词后缀或名词后缀
2）常见词扩展：
sanative adj.有疗效的,有益于健康的
sedative adj. 镇静的
 
9. expurgate v.订正，删除
1）前缀：ex-表示向外；
2）词根：purg- 表示冲洗；
3）后缀：－ate动词后缀或形容词后缀
```

#  第16节:词汇记忆之推动儿童教育 让世界免受惩罚
##  一、Ped=child, education,表示“儿童”，引申为“教育”
词汇助记：pediatrics

```
1. pedology n.儿科学
1）后缀：－ology 学科名词后缀
2）熟词强化：psychology心理学；chronology年代学；sociology社会学；
3）常见词扩展：
theology n. 神学；
anthropology n.人类学
 
2. pediatrist n.儿科医生
1）后缀：-ist名词后缀，表示人；
2）熟词强化：linguist 语言学家；sophist博学之人
3）常见词扩展：
optometrist n.验光师
egoist n.利己主义者
terrorist n.恐怖分子
 
3. pedant n.迂腐的教师，书呆子
1）前缀：ant 形容词或名词后缀
2）常见词扩展：
deodorant n. 除臭剂
inhabitant n. 居民
 
4. pedantry n.迂腐的，卖弄学问
1）后缀：－ry表名词，多放在一个完整的单词后面；
2）常见词扩展：
rivalry n.竞争，敌对
husbandry n.农耕
poetry n.诗歌
 
5. pedagogy n.教学法，教育学
1）词根：agog- 表示引导；
2）常见词扩展：
demagogue n.煽动者
```

##  二、pen,pun=penalty,表示“处罚”
词汇助记：punish惩罚
 
```
1. penance n.赎罪，自我惩罚
1）后缀：－ance名词后缀；
2）常见词扩展：
vigilance n.警惕
resemblance n.相像
 
2. penal a.刑的，刑罚的
 
3. penalty n.惩罚
 
4. penalize v.宣告有罪，处罚
1）后缀：－ize动词后缀；
2）常见词扩展：
neutralize v.使中和
aggrandize v.扩张
 
5. penitent a.后悔的
1）后缀：-ent动词名词，形容词
2）常见词扩展:antecedent n.先行者
 
6. repent v.后悔，懊悔
 
7.punish v.惩罚
1）后缀：-ish动词后缀，形容词后缀和名词后缀；
2）常见词扩展：
snobbish adj.势利眼的（snob势利眼）
flourish v.繁荣（flour花+ish）
lrish a.爱尔兰的n. 爱尔兰语
 
8. impunity n.不受刑罚，免罚
1）前缀：im-表示否定
2）常见词扩展：
impassive adj.无动于衷的
innocuous adj.无害的
```

##  三、pel,puls= drive, push, 表示“驱动，推”
 
```
compel v.强迫
compeling adj.激发兴趣的，令人信服的，有说服力的
expel v.开除，驱逐
impel v.推进，驱动
dispel v.驱散（烟雾等）
propel v.推进
propeller n.推进器
repel v.抵制，击退（敌人），使…反感
compulsion n.强制
compulsory a.强制性的
expulsion n.驱逐
pulse n.脉搏
pulsate v.（脉）搏动
impulse n.冲动，刺激
impulsive a.冲动的
propulsion n.推进（力）
repulse n.打退，击退
repulsive a.排斥的，击退的
```

#  第17节:词汇记忆之多接触世界 生活才会更充满希望
 
 
##  一、tact, tag, tig, ting = touch表示“接触”
词根助记：contact n.接触；联系
 
```
1. tactual a.触觉的
1）后缀：-al形容词后缀；
2）常见词扩展：
temporal adj.临时的；
terminal adj. 终点的；n.终点；
trivial adj.琐碎的，不重要的
 
2. tactility n.触觉
1）后缀：-ility 名词后缀；
2）熟词强化：
mobility n. 移动性;
servility n. 奴性，过分谦卑；
3）常见词扩展：
civility n. 谦恭，礼貌；(civilization 文明)
hostility n. 敌对
 
3. intact adj.未触动的；完整的
1）前缀：in- 表示否定；
2）常见词扩展：inhuman adj.无人性的，野蛮的；
 
4. tact n.机智；圆滑
 
5. tactics n.战术；策略
1）后缀：-ics 名词后缀，多表学科
2）常见词扩展：dynamics n. 动力学；
 
6. contagion adj.传染，感染
 
7. contagious adj.传染的
 
9. tangible adj. 能触知的;明确的
1）前缀：-ible 形容词后缀；
2）熟词强化：
flexible adj.灵活的；
eligible adj.合格的；
3）常见词扩展：
invincible adj.不可战胜的；
 
10. intangible a.不能触知的
 
```

##  二、sper = hope, 表示“希望”
词根助记：desperate adj. 绝望的

```
1. desperation n. 绝望；拼命
2. desperado n.亡命徒
1）后缀：－ado 名词后缀；
2）常见词扩展：tornado n. 龙卷风；
 
3. despair v.绝望
4. prosper v.繁荣
5. prosperous adj.繁荣的
6. prosperity n.繁荣昌盛
7. asperity n.粗糙；严厉,环境艰苦；
```

#   第18节:词汇记忆之听花开的声音
 
##  一、flor, flour=flower,表示“花”
词汇助记： 音变关系u-v-w
 
```
1. floral adj.花的，似花的
1）后缀：-al 形容词后缀；
2）常见词扩展：
mortal adj.致命的；
mutual adj. 相互的，共同的；
 
2. floret n.小花
1）前缀：-et 名词后缀，表示小东西
2）常见词扩展：circlet n. 小圈，小环；
 
3. florid adj.（文章）华丽的
1）后缀：－id 形容词后缀；
2）常见词扩展：
fluid adj. 流动的；
invalid adj. 无效的；
 
4. florescence n.开花期
1）后缀：－escence 名词后缀
2）常见词扩展：
candescence n. 发光
luminescence n. 发光
 
5. floriculture n.花卉栽培
 
6. defloration n.摘花；奸污
1）前缀：de- 否定，向下, 强化；
2）常见词扩展：define 下定义；
 
7. effloresce n.（花）开放;
1）后缀：-esce 名词后缀；
2）常见词扩展：effervesce n. 冒气泡，沸腾；
 
8. flourish v.繁荣，兴旺
flourishing adj.兴旺的
```

##  二、voc, vok = call voice，表示“叫喊，声音”
词汇助记：voice 音变——voc, vok
 
```
1. vocal adj.声音的
2. vocalist n.歌手
3. vocation n.职业；使命
4. avocation n.副业,嗜好
5. advocate v.拥护；倡议
1）后缀：-ate 动词后缀或者形容词后缀；
2）常见词扩展：
attenuate v.变薄，变弱
effeminate adj.柔弱的
 
6. advocator n.拥护者,提倡者
1）后缀：-ator 复合名词后缀；
2）常见词扩展：
gladiator n.角斗士
dictator n.独裁者
 
7. convoke v.召集
1）前缀：con- 表示全部，共同，所有；
2）常见词扩展：
congregate v.使结合，聚集
contemporary adj.同时代的，同一时期的
```

#  第19节:词汇记忆之接触群体 增长智慧
继续上节：
```
8. convocation n.召集，集会
1）后缀：-ation 名词后缀；
2）常见词扩展：
agitation n.鼓动，煽动
emancipation n.解放
 
9. revoke v.取消，撤回
 
10. revocable a.可废除的，可取消的
 
11. irrevocable a.不能改变的
1）前缀：I + 辅音＋相同辅音，从两个辅音中间断开表示否定；
2）常见词扩展：
ir-reversible a.不能改变的
irrecon-cilable a.不可调和的
 
12. vociferous a.喧闹的
1）词根：fer- 带来；
2）常见词扩展：aquiferous adj.含水的
 
13. evoke v.唤醒，唤起
 
14. equivoke n.双关语
 
15. equivocal a.双关语；模陵两可的
1）词根：equ- 表示相等；
2）常见词扩展：
equivalent a.相等的
equilateral a.等边的
 
17. invoke v.请求，使[使[法规]生效
 
18. invocatory adj.祈求的
 
19. provoke v.触怒，煽动
 
20. provocative adj.激怒人的，激发人的
``` 

##  一、gnos(t),gnor=know,表示“知道”
词根助记：
ignore v.疏忽；不理睬
g-h-k 音变；gn—kn
 
```
1. gnosis n.灵知
1）后缀：－sis 名词后缀；
2）常见词扩展：heliosis n. 中暑；
hypnosis n. 催眠
 
2. diagnosis n.诊断
1）前缀：dia- 表示在…之间；
2）熟词强化：
diameter n. 直径；
diagonal n.对角线；
 
3. prognosis n.预测，诊断
1）前缀：pro- 向前；
2）熟词强化：projector n. 投影仪；propel v. 推进，推动；
3）常见词扩展；procrastinate v. 拖延；
 
4. prognostic adj.预兆的 n.征兆
1）-ic 形容词后缀或者名词后缀；
2）常见词扩展：
static adj. 静态的；
chaotic adj. 混乱的，无序的；
 
5. agnostic adj.不可知论的
 
6. ignorance n.无知，愚昧
1）后缀：－ance 名词后缀；
2）常见词扩展：surveillance n. 监视；
endurance n.忍耐力；
repugnance n. 抵触，反感，厌恶；
``` 

##  二、greg=group,表示“群体”
 
```
1. gregarious adj.群居的；爱社交的
1）后缀：－arious 形容词后缀；
2）常见词扩展：
nutritious adj. 有营养的；
arduous adj.努力的，费力的；
dubious adj.半信半疑的；
 
2. aggregate v.合计；聚集
1）后缀：－ate 动词，形容词后缀；偶尔也表名词；
2）常见词扩展：
confiscate v. 没收，充公；
fluctuate v. 波动；
contemplate v. 沉思
 
3. aggregation n.聚集物
 
4. congregate v.聚集
1）前缀：con-全部，共同，所有
2）常见词扩展：
concession n. 妥协，让步；
confabulate v. 交谈；
 
5. disaggregate v.分解；分离
1）前缀：dis- 否定，相似处发散；
2）熟词强化：
discuss 讨论；
concussion 脑震荡，震荡冲击；
3）常见词扩展：dismount v. 下车，下马；
 
6. egregious adj.过分的；不寻常的
 
7. segregate v.分开，隔离
1）前缀：se- 表示分开；
2）熟词强化：separate 分离
3）常见词扩展：seclude v. 隔绝，隐退；
secede v. 脱离，退出；
 
8. segregation n.（种族）隔离
```

#  第20节:词汇记忆之相信说话的力量
 
##  一、reed,cred = believe,trust,表示“相信,信任”
 
```
1. credence n.相信；信用
1）后缀：-ence
2）常见词扩展：
eminence n. 杰出，著名
turbulence n.骚乱
 
2. credit n.信任；学分
 
3. credible adj.可靠的，可信的
1）后缀：－ible 形容词后缀
2）常见词扩展：
comprehensible adj. 可以理解的
irresistible adj. 不可抵抗的，无法抵抗的
 
4. accredit v.委任；信任
 
5. discredit v.不信任，怀疑
1）前缀：dis- 否定，相似处发散；
2）常见词扩展：
disperse v.散布，分散
dysphoria n. 烦躁不安；
dyspepsia n.消化不良
dyslexia n.阅读障碍
 
6. credulous adj.轻信的
7. incredulous adj.不轻信的
8. incredulity adj."不轻信
```

##  二、 dict, dic= say, assert, 表示“说话，断言”
dictionary n.字典
 
```
1. dictate v.口授；命令；听写
1）后缀：－ate 动词、形容词、名词
2）常见词扩展：
coagulate v.凝结；
eradicate v.根除；
 
2. dictator n.发命令者；独裁者
1）后缀：－ator 名词后缀；
2）常见词扩展：
agitator n. 鼓动者
incubator 孵化器；早产儿育儿箱
 
3. dictum n.格言；断言
4. benediction n.祝福
1）前缀：bene- 表示好
2）常见词扩展：
benevolent adj.善心的
benefaction n.好事；恩惠
 
5. malediction n.恶言；诅咒
1) 前缀：male- 表示 坏
2）常见词扩展：
malnutrition n.营养不良（mal+nutrition营养）
malediction n.诅咒（male+dict说+ion→说坏话）
malefaction n.坏事，恶行（male+fact做+ion→做坏事）
malodor 恶臭（mal+odor气味）
maltreat v.虐待（mal+treat对待）
 
6. contradict n.反驳；否认
1) 前缀：contra- 相反；
2）常见词扩展：
contravene v.违反，取消
contraband v.走私
 
7. contradictory adj.矛盾的，对立的
 
8. jurisdiction n.司法权
 
9. predict v.预言；预告
1）前缀：pre- 向前；
2）常见词扩展：
premonish v.预告，预感
prescient adj.有先见之明的
 
10. prediction n.预言；预告
 
11. indict v.起诉，控诉
 
12. indictment n.起诉，控诉
 
13. abdicate v.放弃；退位
1) 前缀：ab- 否定；
2）常见词扩展：abstinence n.节制，戒
 
14. dedicate v.献身于，致力于
 
15. indicate v.指示；表示
 
16. vindicate v.辩护；拥护
1) 词根：vin- 表示胜利，赢；
2）熟词强化；invincible adj.不可战胜的
 
17. addict v.耽溺，上瘾
 
18. addiction n.沉溺，嗜好；吸毒成瘾
 
19．verdict n. 裁判，判决
1)词根：ver- 真实；
2）常见词扩展：
verify v.证实；
veracious adj.诚实的；
```

#  第21节:词汇记忆之叫喊的力量
 

##  一、fore, fort = storng, 表示“强大，力量”
词汇助记：force n.力量，强制力
 
```
1. forceful adj.有力的
2. enforce v.强行；实行
3. reinforce v.加强，巩固
4. fort n.要塞，堡垒
5. fortress n.堡垒
6. fortify v. 加固，加强
7. fortitude n.刚毅，不屈不挠
8. effort n.努力，奋发
9. comfort v.安慰；舒适
10. comfortable a.愉快舒适的
11. discomfort n.不安，不愉快
```

##  二、claim, clam= shout,表示“呼喊，叫喊”
词汇助记：claim 要求；论断
 
```
1. clamor 喧哗；吵闹
1）前缀：-or 名词后缀；
2）常见词扩展：
precursor n.先驱
detector n.探测器
mentor n.导师
 
2. clamorous adj.吵闹的
1）后缀：－orous 复合形容词后缀；
2）常见词扩展：
vacuous adj.无意义的
assiduous adj.勤勉的
 
3. acclaim 欢呼；喝采
 
4. acclamation 喝采，称赞
1）后缀：－ation 名词后缀；
2）常见词扩展：
collaboration n. 合作，协作；
emancipation n. 解放
excruciation n. 酷刑，惨刑
 
5. declaim 抨击；
1）前缀：de- 向下，否定，加强；
2）常见词扩展：
decrypt v.解密
deduce v.推论，演绎
degrade v.降级；退化
 
6. declamation 慷慨演说；雄辩
 
7. exclaim 叫喊；大声说
1）前缀：ex- 向外；
2）常见词扩展：
exhibit v.展览；显示
expatriate v.把…逐出国外
expedite v.加快，促进
 
8. exclamation 叫喊；感叹
 
9. proclaim v.公布；声明
1）前缀：pro- 向前；
2）常见词扩展：
prostitute n. 卖淫者
protrude v. 伸出，突出 (intruder n.入侵者)
 
10. proclamation 宣告；宣言
 
11. reclaim 收回
1）前缀：re- 往回；
2）常见词扩展：
revenge v. 报仇
revise v. 修正，改订
```

#  第22节:清清楚楚 不犯错误
 
 
##  一、clear,clar,clair=clear,bright,表示“清楚，明白”
 
```
1. clearance 清算；清除
2. clarify 澄清，弄清
1）后缀：-ify 动词后缀；
2）常见词扩展：
amplify v. 放大；
calcify n.钙化；僵化
 
3. clarity 清楚
1）后缀: -ity 名词后缀；
2）常见词扩展：
density n 比重；浓缩
integrity n. 完整，完全, 正直，诚实
 
4. clarification 澄清；纯化
5. declare 宣称；断言
6. declaration 定言；陈述
7. clairaudient 有超人听力的
1) 词根：audi- 表示听；
2）常见词扩展：
audience n.听众；观众
audible adj.听得见的；可听的
inaudible adj.听不见的
auditorium n.礼堂；讲堂
``` 

##  二、err=wander,mistake,表示“漫游，犯错误”
error n.错误（err+or）
 
```
1.errant adj.漫游的；错误的
1）后缀：-ant 形容词后缀或者名词后缀；
2）常见词扩展：
hydrant n.水龙头
pendant n.下垂物 adj.吊着的
vigilant adj.警惕的
 
2. erratum n.正误表
1）后缀：-ium 名词后缀；
2）常见词扩展：
calcium n钙
auditorium n. 礼堂；讲堂
solarium n.日光浴室
 
3. erroneous adj.错误的
1）后缀:-ous 形容词后缀；
2）常见词扩展：
perfidious adj.背信弃义的
fugacious adj.易散去的，转瞬即逝的
hydrous adj.含水的，水状的
anhydrous adj.无水的
 
4. erratic adj.古怪的 n.怪人
1）后缀：-atic 复合后缀；表示形容词或者名词；
2）常见词扩展：
static adj.静止的
chromatic adj. 彩色的
monochrome adj. 单色的
chromosome n.染色体
somatic adj.躯体的，肉体的
 
5. aberrant adj.越轨的；异常的
1）后缀：－ant 形容词后缀或者名词后缀；
2）常见词扩展：
vibrant adj. 震动的；有活力的
vibrate v. 摇动，震动
vibration n. 振动，摇动
 
6. aberration n. 越轨；失常
7. inerrancy n.无错误
```

##  三、sol = sun ,表示“太阳”
solar adj.太阳的

```
1. solarize v.晒太阳（solar太阳的＋ize）
1）后缀：-ize 动词后缀；
2）常见词扩展：
summarize v. 概括
agonize v.感到苦恼
agony n. 痛苦
 
2. parasol n.遮阳伞
1）前缀：para- 在旁边；
2）常见词扩展：paradox n.勃论，相矛盾
 
3. solarium n.日光浴室
1）后缀：－ium
2）常见词扩展：stadium n.露天大型体育场；
 
4. insolate v.曝晒
5. solace n.安慰
6. console v.安慰
```

#  第23节:交叉欺骗
 
##  一、cruc, crus, crux = cross, 表示“十字形，交叉”

```
1. crucial adj.关键的；十字行的
1）后缀：－ial 形容词后缀；
2）常见词扩扩展：diluvial adj.大洪水的
3）词根扩展：lav,luv,lut=wash,表示“洗,冲洗”
lavish v.浪费adj. 浪费的
lavatory n.洗手间
ablution n.沐浴；净化
dilute v.冲淡；稀释
dilution n.冲淡；稀释物
 
2. cruciform adj.十字行的
1）词根：form- 表示形状；
2）常见词扩展：
transform v.变形；改造
uniform adj.一致的；n. 制服
formulate v.公式化；系统陈述
conform v.使一致；遵从
deform v.使残废；变形
 
3. crucify v. 钉在十字架上；折磨
4. crusade n.十字军；改革运动
5. crusader n.十字军战士；改革者
6. excruciate v. 受酷刑，拷打
1）前缀：ex- 表示向外；
2）常见词扩展：
excurse n. 远足，旅游
exempt v.免除，豁免
exemption n.免除，豁免
 
7. excruciation n.酷刑，惨刑
8. cruise v.巡航；徘徊
1）后缀：－ise 动词后缀或者名词后缀；
2）常见词扩展：jeopardize v.危害
 
9. cruiser n.巡洋舰
10. crux n.难点；关键
```

##  二、fall, fail, fault = err, deceive, 表示“犯错误，欺骗”

```
1. fallacy n. 错误；谬论
1) 后缀：-acy 名词后缀；
2）常见词扩展：
accuracy n.正确，准确
supremacy n.至高无上
intimacy n.亲密
 
2. fallacious adj. 犯错误的
1）后缀：-acious 复合形容词后缀；
2）常见词扩展：
perspicacious adj. 有判断力的，有洞察力的；
rapacious adj. 强夺的
 
3. fallible adj.易错的
1）后缀：-ible 形容词后缀；
2）常见词扩展：gullible adj. 受骗的
 
4. infallible adj. 无可指责的
5. falsehood n.虚伪；错误
1）后缀：-hood 名词后缀；
 
6. falsify v.篡改；欺骗
7. falsification n.伪造；曲解
1）词根：fic- 表示做
2）常见词扩展：artificial 人工的；矫揉造作的
 
8. failure n.失败
1）后缀：-ure 名词后缀；
2）常见词扩展：conjecture n.推测，猜想
 
9. fault n.过失，缺点
10. default v/n. 拖久（债）；违约；弃权；缺席
```

#  第24节:词汇记忆之一味追求金钱 一定会变的尖酸刻薄
 
 
##  一、pend,pens,pond=weight,expend 表示“称重量，称银子”，引申为“花费”

```
1. expenditure n.支出，开销（expend+iture）
1）后缀：－ure 名词后缀；
2）词根：acupuncture n.针灸
3）词根扩展：acid, acri, acrid, acu=sour, sharp, 表示“尖，酸，锐利”
acid adj.酸的
acidify v.酸化
acidity n.酸度，酸性
acidulous adj.尖酸刻薄的，有讽刺意味的；
acrid adj.辛辣的；尖刻的
acute adj.尖锐的，敏锐的
acuity adj.尖锐，敏锐
punctual adj.准时的
punctuality n.守时
 
2. expensive adj.昂贵的
1）后缀：-ive 形容词后缀或名词后缀；
2）熟词强化：
derisive adj.嘲笑的；可笑的
reclusive adj.隐居的
fugitive adj.逃亡的n.逃亡者
repulsive adj.排斥的，击退的
 
3. pension n.年金；养老金
 
4. compensate v.赔偿；报酬
1）后缀：-ate 动词后缀或者形容词后缀；
2）常见词扩展：perpetuate v.使永久，不朽
3）词根扩展：
petition n.请愿，申请
petitioner n.请愿者
appetite n.欲望；胃口
appetizer n.开胃食品
centripetal adj.向心的
compete v.竞争,竞赛
competent adj.资格的，能力的
competence n.资格,能力
competitive adj.有竞争力的
incompetent adj.无能的
impetus n.动力，冲动
impetuous adj.冲动的
perpetual adj.永久的
petulant adj.(脾气)暴躁的
 
5. recompense v.分配，分送
1）前缀:dis- 两个意思，一个表示否定，一个表示向四处发散；
2）常见词扩展：dissident adj.持不同意见的
3）词根扩展：sid=sit,表示“坐”"
reside v.居住
residence n.居住；住宅
preside v.主持
president n.总统；校长
subside v.沉淀；平息
consider v.考虑
considerate adj.考虑周到的
insidious adj.阴险的
assiduous adj.勤勉的，刻苦的
sedate adj.文静的
sedulous adj.勤勉的
supersede v.代替，取代
 
7. dispensable adj.可分配的；不重要的
8. dispensary n.药房
9. propensity n.倾向；习性
10. pensive adj.思考的
11. ponder v.沉思
12. ponderance n.重要
13. ponderous adj.重的，笨重的
```

#  第25节:词汇记忆之抓住时间 留下名字
 
##  一、hibit = hold,表示“拿住”
exhibit v. 展览；显示

```
1. exhibition n.展览，表演
2. exhibitionist n.爱出风头的人
1）后缀：-ist
2）熟词强化：
optometrist n.验光师
pediatrist n.儿科医生（ped儿童+iatrist专家）
 
3）词根扩展：opt,opto=sight,表示“视力”
optic adj.眼的，视力的
optics n.光学
optometer n.视力计
synopsis n.概要
 
3. inhibit v.禁止，阻止
1）前缀：in- 表示向内，否定；
2）常见词扩展：inornate adj.不加修饰的；朴素的
3）词根扩展：orn=embellish,表示“装饰
adorn v.装饰，修饰
adornment n.装饰
ornament n.装饰品
ornate adj.华丽的；装饰的
 
4. inhibition n.禁止，禁令
 
5. prohibit v.阻止，禁止
1）前缀：pro- 表示向前
2）常见词扩展：propone v.提议，提出
3）词根扩展：pon, pound= put, 表示“放置”
component adj.组成的 n. 部分
opponent v.对立的 n. 对手
proponent n.提议者
exponent n.讲解者
compound n.化合物
postpone v.推迟
expound v.详细说明
impound v.把…围住；扣押
propound v.提出，提议
 
6. prohibition n.禁止
 
7. prohibitive adj.禁止的，价格极贵的
```

##  二、nomin = name,表示“名称，名字”
```
1. nominal adj.名义上的
1）后缀：-al 形容词后缀或名词后缀
2）常见词扩展：rudimental adj.基本的
3）词根扩展：rud= rude, 表示“原始，粗野”
rude adj.原始的；粗鲁的
rudiment n.基本原理,初步
erudite adj.博学的
erudition n.博学
 
2. nominate v.提名；任命
 
3. nominee n.被提名者
 
4. denominate v.给…命名
1）前缀：de- 向下，否定和加强；
2）后缀：－ate 动词后缀；
 
5. denomination n.命名；（货币）单位
```

#  第26节:词汇记忆之无常故苦 苦故无我 无我故空 一切皆空
 
##  一、vac, van, void = empty，表示“空”
avoid v.回避
vacation n.假期，休假

```
1. vacuum n.空虚；真空吸尘器
后缀：-uum 名词后缀
 
2. vacuous adj.空虚的；无意义的
1）后缀：－ous 形容词后缀；
2）常见词扩展：precocious a早熟的
3）词根扩展：coct= cook，表示“煮，调配”
concoct v.编造，虚构
decoct v.煎，熬
precocity n.早熟
 
3. vacant adj.空的；空闲的
1）后缀：-ant 形容词后缀或名词后缀
2) 常见词扩展：vibrant a.震动的；有活力的
3）词根扩展：vibr=swing，表示“摇摆”
vibrate v.摇动，震动
vibration n.振动，摇动
vibrant adj.震动的；有活力的
revibrate v.震动
 
4. vacate v.弄空；退出
1）后缀：－ate 此处为动词后缀；
2）常见词根扩展：satiate v.使充分满足
3）词根扩展：sati, satis, satur = enough, full of food, 表示“足够，饱足”"
satisfy v.使满足
satisfaction n.满足
satisfactory a.令人满意的
satiable adj.可使满足的
insatiate adj.不满足的
saturate v.使饱和
saturation n.饱和
saturated adj.饱和的
 
5. evacuate v.腾空；清除；撤离
6. evacuation n.撤离，撤退
7. vanity n.空虚；虚荣心
1）后缀：－ity 名词后缀；
2）后缀扩展：
equanimity n.镇定，沉着
sanity n.神志清楚
3) 词根扩展： san, sanit= healthy,表示“健康的”
sane adj.神志清醒的
insane adj.头脑不清的
sanitary adj.卫生的
sanitation n.卫生（设备）
sanative adj.有疗效的,有益于健康的
sanatorium n.疗养院
 
8. vanish v.不见，消失
1）后缀：－ish 动词，名词或者形容词后缀；
2）常见单词扩展：languish v.卷怠，衰弱
3）词根扩展：langu=faint，表示“虚弱”
languid adj.卷怠的
languor n.衰弱，憔悴
 
9. void adj.空的，n.空虚
10. avoidance n.避免；无效（avoid+ance）
11. devoid adj.无的，缺…的（de加强意义+void→空的→无的）
```

#  第27节:词汇记忆之相信爱
##  一、am, amor, amat=love,表示“爱，情爱”

```
1. amatory adj.恋爱的；色情的
1）后缀：-atory 复合后缀；
2）熟词强化：lavatory n.洗手间
3）常见词扩展：migratory adj.移居的，漂泊的
4）词根扩展：migr=remove,表示“迁移”
migrate v.迁移，移居
migrant n.候鸟，迁移者；adj.移居的，流浪的；
emigrate v.移居（外国）
immigrate v.移居；入境
migraine n.偏头痛
 
5）后缀：－ine 扩展；
6）常见词扩展：turbine n.涡轮
7）词根扩展：turb= stir“表示”搅动“
disturb v.扰乱，打扰
disturbance n.骚动，动乱
turbid adj.混浊的
turbulent adj.骚乱的
turbulence n.骚乱
perturb v.骚乱
 
2. amiable 和蔼的，友善的
 
3. amicable 友好的，温和的
1）后缀：－able 形容词后缀；
2）常见词扩展：culpable adj. 有罪的；应受指责的
3）词根扩展：culp=fault，表示“错，罪”
inculpate v.归罪
exculpate v.无罪释放
 
4. amity 友好，和睦
1）后缀：-ity 名词后缀；
2）常见词扩展：indemnity n.赔偿（物）
3）词根扩展：demn=harm表示“伤害”
condemn v.谴责，指责
condemnation n.谴责
indemnify v.赔偿
 
5. enmity 敌意，仇恨
```

##  二、cur=care,表示“关心”
 
```
1. curious adj.好奇的
2. curiosity n.好奇的；奇品
3. curio n.小件珍奇物品;古董
4. secure adj.安全的
5. security n.安全；证券
6. cure v.医治，治疗
7. curable adj.可治好的
8. curative adj.治病的，（药）有效的
9. accurate adj.准确的，精确的
10. accuracy n.正确，准确
11. procure v.取得；引起
12. procurable adj.可得到的;得手的
13. procurement n.获得
14. curator n.(博物馆等)馆长
```

#  第28节:词汇记忆之大德曰生 珍爱生命
 
##  Viv, vit, vig=life，表示“生命”

```
1. vivid adj.活泼的；生动的
1）后缀：-id 形容词后缀；
2）常见词扩展：horrid adj.可怕的
3）词根扩展：hor(r)=shudder,dread，表示“颤抖，害怕”
abhor v.憎恨
abhorrent adj.憎恶的
horrible adj.可怕的
horrify v.使恐怖
 
2. vivify v.给予生气；鼓励
1）后缀：-ify 动词后缀；
2）常见词扩展：nullify v.取消，废除
3）词根扩展：null=none，表示“没有”
null adj.无效的，等于零的
annul v.取消，废除
 
3. vivisect v.活体解剖
1）词根：sect- 表示切割
2）词根扩展：sect,seg= cut, divide, 表示“切， 割”
section n.切开；部门
bisect v.二分
dissect v.解剖，切开
dissection n.解剖；细查
intersect v.横切;交叉
intersection n.横切；交叉
segment n.部分；切片
segmentation n.分裂；切断
 
4. convivial adj.欢乐的；欢宴的
1）前缀：con- 全部，共同，所有；
2）常见词扩展：consent v.同意，答应
3）词根扩展：sens, sent =feel, 表示“感觉”
sense n.感觉
sensible adj.能感觉的；明智的
sensibility n.敏感性；感情
sensitive adj.敏感的
sensory adj. 感觉的，知觉的
sentiment n.感情
sentimental adj.多感情的；感伤的
nonsense n.无意义的话
assent n.赞成，附和
dissent v.持不同意见
dissension n.意见不合，冲突
consensus n.一致意见
resent v.怨恨
 
5. revive v.复活
1）前缀：re- 表示往回或者反复；
2）常见词扩展：resign v.辞职；放弃
 
6. revival n.复活
7. vivacious adj.活泼的；有生气的
8. vital adj.有活力的；重要的
9. vitality n.活力
10. vitalize v.赋予生命
11. revitalize v.恢复生命，复兴
12. vitals n.重要器官；核心
13. viable adj.能活的
14. viability n.生存能力
15. vigor n.精力
16. vigorous adj.精力旺盛的
17. invigorate v.鼓励；使强壮
18. invigorating adj.激发精神的
19. vigilant adj.警惕的
20. vigilance n.警惕，警戒
```

#  第29节:词汇记忆之脚
##  ped=foot 表示“脚”
 
```
1. pedal adj.脚的，足的
1）后缀；-al 名词后缀或者形容词后缀
2）常见词扩展：territorial adj.领土的（territory+al）
3）词根扩展：terr=eartth 表示“土地”
terrain n.地形；地域
terrace n.平台；梯田
territory n.领土，领域
Mediterranean n.地中海
subterrane n.地下层，地下室
subterranean adj.地下的
inter v.埋葬
disinter v.挖出
 
2. pedestrian adj.步行的 n.步行者
1）后缀：-ian 形容词或名词
2）熟词强化：
historian n.历史学家
civilian n.平民
 
3. biped adj.两足的 n.两足动物
1）前缀：bi- 表示二
2）常见词扩展：bigamy n.重婚罪
3）词根扩展：gam=marriage，表示“婚姻”
monogamy n.一夫一妻制
misogamy n.厌恶结婚
 
4. quadruped v.加快，促进
1）后缀：-ite 动词后缀；形容词和名词
2）常见词扩展：
polite adj.礼貌的
definite adj.明确的
erudite adj.博学的
recondite adj.深奥的
parasite n.寄生虫，食客
ignite v.点燃
unite v.联合，统一
3）词根扩展：cond =hide,表示“藏”
abscond v.潜逃
 
4）词根扩展：fin=end,boundary,表示“结束，范围”
finish v.完成，结束
finalist n.决赛选手
finality n.最后，定局
finite adj.有限的
infinite adj.无限的
infinitude n.无限，无数
confine v.限制，监禁
confined adj.有限制的； 狭窄的
define v.限定；下定义
definition n.定义；（轮廓）清晰
definitive adj.决定性的
 
7. expeditious adj.迅速的
8. expedition n.远征，探险
9. impede v.妨碍，阻碍
10. impediment n.阻碍，阻碍物
11. expedient adj.方便的；权宜的
12. podiatrist n.足病医生
```

#  第30节回顾总结大结局

```
Ambul = walk, 表示“行走，走路”
Anim = life,spirit, 表示“生命，精神”等
av, avar, avi = desire,bird,表示“渴望，鸟”
bio,bi = life,表示“生命，生物”
Cit = quote, call, 表示“引用，唤起”
card, cord = heart,表示“心脏，一致”
ann, enn = year, 表示“年，一年”
chron = time, 表示“时间”
fug = flee,表示“逃，离开” 
her, hes = stick, 表示“粘附” 
fer = bring, carry, 表示“带来，拿来” 
erg = energy,work,表示“能量，活动” 
flu = flow,表示“流动” 
Liter = letter,表示“文字,字母” 
meter, metr, meas, mens = measure,表示“计量，测量等” 
soc = companion,表示“同伴”，引申为“社会” 
clear, clar, clair = clear, bright,表示“清楚，明白”
cid，cis = cut, kill, 表示“切开，杀”
astro, aster = star, 表示“星星，天空”
spir = breathe, 表示“呼吸” 
rect = stright, right, 表示“正，直”" 
rid, ri s= laugh,表示“笑” 
scrib,script = write,表示“写” 
stell = star,表示“星星” 
mob = move,表示“动” 
Soph = wise, 表示“智慧，聪明” 
spect, spic = look, see,表示“看”
ras, rad = scrape,表示“刮擦” 
limin, lim = threshold，表示“门槛，限制” 
path = feeling,suffering,illness,表示“感情，痛苦，病”
per = try,表示“尝试” 
Lingu = language ,表示“语言”，原意为“舌头” 
fid = trust,faith,表示“相信，信念” 
doc,doct = to teach, 表示“教” 
Fract = break, 表示“打碎” 
pur, purg = pure, 表示“纯洁” 
Ped = child, education, 表示“儿童”，引申为“教育” 
pen, pun = penalty, 表示“处罚”" 
pel, puls = drive, push, 表示“驱动，推” 
tact, tag，tig，ting = touch表示“接触” 
sper = hope, 表示“希望” 
flor, flour = flower,表示“花”
voc, vok = call voice，表示“叫喊，声音” 
creed, cred = believe, trust,表示“相信,信任” 
dict, dic = say, assert, 表示“说话，断言”" 
claim, clam = shout,表示“呼喊，叫喊”
fore, fort = storng, 表示“强大，力量” 
clear, clar, clair = clear, bright,表示“清楚，明白”
err = wander,mistake,表示“漫游，犯错误” 
sol = sun ,表示“太阳” 
cruc, crus, crux = cross, 表示“十字形，交叉”
lav,luv,lut = wash,表示“洗,冲洗”
fall, fail, fault = err, deceive, 表示“犯错误，欺骗”" 
pend,pens, pond = weight, expend, 表示“称重量，称银子”，引申为“花费”" 
acid, acri, acrid, acu = sour, sharp, 表示“尖，酸，锐利”
sid = sit,表示“坐”"
hibit = hold,表示“拿住” 
opt,opto = sight,表示“视力” 
nomin = name,表示“名称，名字” 
vac, van, void = empty，表示“空” 
sati, satis, satur = enough, full of food, 表示“足够，饱足”"
vibr = swing，表示“摇摆”
coct = cook，表示“煮，调配” 
am, amor, amat = love,表示“爱，情爱”
cur = care,表示“关心” 
migr = remove,表示“迁移” 
turb = stir“表示”搅动“ 
culp = fault，表示“错，罪” 
Viv, vit, vig = life，表示“生命” 
hor(r) = shudder，dread，表示“颤抖，害怕” 
null = none，表示“没有” 
sect,seg = cut,divide, 表示“切，割” 
sens, sent = feel, 表示“感觉” 
ped = foot, 表示“脚” 
terr = eartth 表示“土地” 
gam = marriage，表示“婚姻” 
fin = end,boundary,表示“结束，范围”

```


